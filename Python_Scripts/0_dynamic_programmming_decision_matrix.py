# -*- coding: utf-8 -*-
""" Computes a matrix that gives, for each time step, the best patch to choose in order to optimize the fitness of an individual
...
def fitness(x_state, i, f_vectors):
    ...
    i -- specific patch
    f_vectors -- enables the calcultation of fitnesses at time t using the ones in time t+1
    
    Returns w_patch -- the fitness calculated
    """
    # First we calculate the possible states of a forager at the end of a time step.
    ...
    # To do so we ponderate the two possible states with the probability of their happening.
...
def over_states(time, mat_best_patch, fxtt, f_vectors):
    ...
    Arguments:
    time
    mat_best_patch -- stores the best patch choice for each state value of each time
...
p_mortality = [0, 0.004, 0.02]  # Probability to die from a cause other than starvation in the 3 patches
...
def main():
    ...
    # The survival at time = HORIZON is 1 for all state > X_CRITICAL because we are only interested in individuals that are alive at the end of the time period.
    f_vectors = np.zeros([X_MAX, 2])
    f_vectors[list(range(X_CRITICAL, X_MAX)), 1] = 1
    
    # Initialisation of the best_patch and fxtt matrixes which keep for each time and each state the best_patch to choose and the fitness associated.
    fxtt = np.zeros([HORIZON, X_MAX])
    mat_best_patch = np.zeros([HORIZON, X_MAX])
    
    # We go back in time from t=HORIZON, when we know individuals are alive, to t=1,
