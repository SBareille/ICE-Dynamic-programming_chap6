...
def over_patches(x_state, f_vectors):
    ...
    return [f_vectors[index - 1, 0], best_patch, choice]
...
def main():
    ...
    choices = np.zeros([HORIZON, INDEX_MAX])
    ...
    mat_best_patch[HORIZON - 1, :] = list(range(1, INDEX_MAX+1))
    fxtt[HORIZON - 1, : ] = list(range(1, INDEX_MAX+1))
    choices[HORIZON - 1, : ] = list(range(1, INDEX_MAX+1))
