...
def fitness(x_state, benefit_row_i, p_benefit_row_i, f_vectors):
    ...
    """
    w_patch = 0 # Initialisation of the fitness variable.
...
if __name__ == "__main__":
    ...
    a,b,c = main()
    print("Best patch matrix :\n", a,
          "\n\n Fitness matrix :\n", b,
          "\n\n Existence of Choice matrix :\n", c)
    
    plt.clf()
    fig = plt.figure()
    
    plt.subplot(1,3,1)
    im = plt.imshow(a[0:30,], cmap = 'gray', vmin = 1, vmax = 4)
    plt.colorbar(im, orientation = 'horizontal')
    plt.subplot(1,3,2)
    im2 = plt.imshow(b[0:30], cmap = 'gray', vmin = 0, vmax = 1)
    plt.colorbar(im2, orientation = 'horizontal')
    plt.subplot(1,3,3)
    im3 = plt.imshow(c[0:30], cmap = 'gray', vmin = 0, vmax = 1)
